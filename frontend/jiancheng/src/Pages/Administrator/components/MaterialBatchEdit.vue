<template>
    <div class="material-batch-edit">
        ...
        <el-dialog v-model="zipperPairDialogVisible"
            title="配对组设置（拉链/拉头、鞋眼/垫片、帽或饰扣或登山扣/钉）"
            width="860px" destroy-on-close>
            <el-alert type="info" :closable="false" style="margin-bottom:12px">
                为需要配对的行指定配对组编号（1-9）。同一配色下编号相同的拉链与拉头、帽（或饰扣、登山扣）与钉视为一对。帽钉/饰扣钉/登山扣钉不填编号时将分开采购（各自成行）。
            </el-alert>
...
export default {
    ...
    computed: {
        ...
        zipperPairWarnings() {
            ...
            const validateCapNail = names.some(n => (n.includes('帽') || n.includes('饰扣') || n.includes('登山扣')) && !n.includes('钉'))
                                 && names.some(n => n.includes('钉') && !n.includes('帽') && !n.includes('饰扣') && !n.includes('登山扣'))
            ...
            for (const r of this.zipperPairRows) {
                ...
                const isCap = (name.includes('帽') || name.includes('饰扣') || name.includes('登山扣')) && !name.includes('钉')
                const isNail = name.includes('钉') && !name.includes('帽') && !name.includes('饰扣') && !name.includes('登山扣')
                ...
                if (pairType === 'capnail' && !validateCapNail) continue
                const key = `${r._ostId}|${r.pairId}|${pairType}`
                if (!byGroup[key]) byGroup[key] = {
                    hasA: false, hasB: false,
                    labelA: pairType === 'zipper' ? '拉链' : pairType === 'eyelet' ? '鞋眼' : '帽/饰扣/登山扣',
                    ...
                    if (!merged[key]) {
                        merged[key] = {
                            ...
                            pairRole: name.includes('拉链头') || name.includes('拉头') ? '拉头'
                                    ...
                                    : name.includes('鞋眼') ? '鞋眼'
                                    : name.includes('垫片') ? '垫片'
                                    : ((name.includes('帽') || name.includes('饰扣') || name.includes('登山扣')) && !name.includes('钉')) ? '帽/饰扣/登山扣'
                                    : (name.includes('钉') && !name.includes('帽') && !name.includes('饰扣') && !name.includes('登山扣')) ? '钉'
                                    : name,
