    ...
    <el-row :gutter="20" class="mt-3">
        <el-col :span="24">
            <el-table :data="tableData" border stripe height="50vh" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="工厂型号">
                    <template #default="{ row }">
                        {{ row.orderShoeTable?.map(s => s.shoeRId).filter(Boolean).join(', ') }}
                    </template>
                </el-table-column>
                <el-table-column label="客户型号">
                    <template #default="{ row }">
                        {{ row.orderShoeTable?.map(s => s.customerProductName).filter(Boolean).join(', ') }}
                    </template>
                </el-table-column>
