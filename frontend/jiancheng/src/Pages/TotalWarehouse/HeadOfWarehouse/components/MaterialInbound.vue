<template>
    ...
    <el-dialog title="入库收据" v-model="isReceiptDialogVisible" width="80%">
        <div v-for="(group, index) in inboundForm.groupedSelectedRows" :key="index">
            ...
            <el-button type="primary" @click="downloadPDF('健诚鞋业入库单', `inboundRecipt${index}`)">下载 PDF</el-button>
        </div>
        <template #footer>
            ...
            try {
                ...
                for (let row of this.inboundForm.groupedSelectedRows) {
                    for (let item of row.items) {
                        if (item.materialCategory == 1) {
                            item.sizeMaterialInboundTable.forEach((element, index) => {
                                item[`amount${index}`] = element.inboundQuantity
                            })
                        }
                        data.push({
