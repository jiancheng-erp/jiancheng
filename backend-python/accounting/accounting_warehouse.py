...
from general_document.accounting_warehouse_excel import (
...
# outbound attrnames
OUTBOUND_RECORD_SELECTABLE_TABLE_ATTRNAMES = [
    "outbound_rid",
    "outbound_datetime",
    "outbound_type",
    "outbound_department",
...
OUTBOUND_RECORD_DETAIL_SELECTABLE_TABLE_ATTRNAMES = [
    "outbound_amount",
]
...
INBOUND_RECORD_SELECTABLE_TABLE_ATTRNAMES = [
    "inbound_rid",
    "inbound_datetime",
    "approval_status",
]
...
# inventory attrnames
INVENTORY_MATERIAL_STORAGE_ATTRNAMES = [
    "inbound_amount",
    "current_amount",
    "unit_price",
...
SELECTABLE_ATTRNAMES = [
    INBOUND_RECORD_SELECTABLE_TABLE_ATTRNAMES,
    SUPPLIER_SELECTABLE_TABLE_ATTRNAMES,
    INBOUND_RECORD_DETAIL_SELECTABLE_TABLE_ATTRNAMES,
...
@accounting_warehouse_bp.route(
    "/accounting/get_warehouse_outbound_record", methods=["GET"]
)
def get_warehouse_outbound_record():
    ...
    if material_model_filter:
        query = query.filter(
            SPUMaterial.material_model.ilike(f"%{material_model_filter}%")
        )
    if date_range_filter_start:
        query = query.filter(
            OutboundRecord.outbound_datetime >= date_range_filter_start
        )
    ...
    response_entities = (
        query.distinct().limit(page_size).offset((page_num - 1) * page_size).all()
    )
    outbound_records = []
    department_mapping = {
        entity.department_id: entity.department_name
        for entity in db.session.query(Department).all()
    }
    for (
        outbound_record,
        ...
        spu,
        material_storage,
    ) in response_entities:
        res = db_obj_to_res(
            outbound_record,
            OutboundRecord,
            attr_name_list=OUTBOUND_RECORD_SELECTABLE_TABLE_ATTRNAMES,
        )
        res = db_obj_to_res(
            outbound_record_detail,
            OutboundRecordDetail,
            attr_name_list=OUTBOUND_RECORD_DETAIL_SELECTABLE_TABLE_ATTRNAMES,
            initial_res=res,
        )
        res = db_obj_to_res(
            spu,
            SPUMaterial,
            attr_name_list=SPU_MATERIAL_TABLE_ATTRNAMES,
            initial_res=res,
        )
        res = db_obj_to_res(
            material,
            Material,
            attr_name_list=MATERIAL_SELECTABLE_TABLE_ATTRNAMES,
            initial_res=res,
        )
        res = db_obj_to_res(
            supplier,
            Supplier,
            attr_name_list=SUPPLIER_SELECTABLE_TABLE_ATTRNAMES,
            initial_res=res,
        )
        res[to_camel("unit_price")] = outbound_record_detail.unit_price
        res[to_camel("outbound_datetime")] = format_datetime(
            outbound_record.outbound_datetime
        )
        res[to_camel("outbound_type")] = format_outbound_type(
            outbound_record.outbound_type
        )
        res[to_camel("outbound_department")] = (
            department_mapping[outbound_record.outbound_department]
            if outbound_record.outbound_department
            else ""
        )
...
def get_warehouse_inventory():
    ...
    material_specification_filter = request.args.get(
        "materialSpecificationFilter", type=str
    )
    ...
    order_shoe_customer_name_filter = request.args.get(
        "customerProductNameFilter", type=str
    )
    ...
    if warehouse_filter:
        query = query.filter(
            MaterialWarehouse.material_warehouse_id == warehouse_filter
        )
    if supplier_name_filter:
        query = query.filter(Supplier.supplier_name.ilike(f"%{supplier_name_filter}%"))
    if material_model_filter:
    ...
    if material_specification_filter:
        query = query.filter(
            SPUMaterial.material_specification.ilike(
                f"%{material_specification_filter}%"
            )
        )
    ...
    if order_shoe_customer_name_filter:
        query = query.filter(
            OrderShoe.customer_product_name.ilike(f"%{order_shoe_customer_name_filter}")
        )
    if shoe_rid_filter:
    ...
    for (
        ...
        res = db_obj_to_res(
            material_storage,
            MaterialStorage,
            attr_name_list=INVENTORY_MATERIAL_STORAGE_ATTRNAMES,
        )
        res = db_obj_to_res(
            spu_material,
            SPUMaterial,
        ...
        res[to_camel("item_total_price")] = round(
            material_storage.current_amount * material_storage.unit_price, 4
        )
...
@accounting_warehouse_bp.route(
    "/accounting/get_warehouse_inbound_record", methods=["GET"]
)
def get_warehouse_inbound_record():
    ...
    warehouse_id_mapping = {
        entity.material_warehouse_id: entity.material_warehouse_name
        for entity in db.session.query(MaterialWarehouse).all()
    }
    ...
    for (
        ...
        res = db_obj_to_res(
            inbound_record,
            InboundRecord,
            attr_name_list=type_to_attr_mapping["InboundRecord"],
        )
        res = db_obj_to_res(
            inbound_record_detail,
            InboundRecordDetail,
            attr_name_list=type_to_attr_mapping["InboundRecordDetail"],
            initial_res=res,
        )
        res = db_obj_to_res(
            supplier,
            Supplier,
            attr_name_list=type_to_attr_mapping["Supplier"],
            initial_res=res,
        )
        res = db_obj_to_res(
            spu,
            SPUMaterial,
            attr_name_list=type_to_attr_mapping["SPUMaterial"],
            initial_res=res,
        )
        res = db_obj_to_res(
            material,
            Material,
            attr_name_list=type_to_attr_mapping["Material"],
            initial_res=res,
        )
        res[to_camel("inbound_datetime")] = format_datetime(
            inbound_record.inbound_datetime
        )
        res[to_camel("approval_status")] = accounting_audit_status_converter(
            inbound_record.approval_status
        )
        res[to_camel("material_warehouse")] = (
            warehouse_id_mapping[inbound_record.warehouse_id]
            if inbound_record.warehouse_id in warehouse_id_mapping.keys()
            else ""
        )
        ...
        res[to_camel("inbound_amount")] = normalize_decimal(
            res[to_camel("inbound_amount")]
        )
        res[to_camel("item_total_price")] = normalize_decimal(
            res[to_camel("item_total_price")]
        )
...
@accounting_warehouse_bp.route(
    "/accounting/get_warehouse_inbound_summery", methods=["GET"]
)
def get_warehouse_inbound_summery():
    ...
    if warehouse_filter:
        inbound_records = inbound_records.filter(
            InboundRecord.warehouse_id == warehouse_filter
        )
    if date_range_filter_start:
        inbound_records = inbound_records.filter(
            InboundRecord.inbound_datetime >= date_range_filter_start
        )
    if date_range_filter_end:
        ### next day
        input_date_time = datetime.strptime(date_range_filter_end, "%Y-%m-%d")
        ...
        query_compare_date = format_date((input_date_time + next_day_delta))
        inbound_records = inbound_records.filter(
            InboundRecord.inbound_datetime <= query_compare_date
        )
    ...
    query = (
        db.session.query(
            ...
            Order.order_rid,
            func.sum(InboundRecordDetail.inbound_amount),
        ...
        .outerjoin(Order, InboundRecordDetail.order_id == Order.order_id)
        .group_by(
            SPUMaterial.spu_material_id, InboundRecordDetail.unit_price, Order.order_rid
        )
    )
    ...
    response_query = (
        ...
        .join(
            InboundRecord,
            InboundRecord.inbound_record_id == time_period_subquery.c.inbound_record_id,
        )
        .join(Supplier, InboundRecord.supplier_id == Supplier.supplier_id)
    )
    ...
    if supplier_name_filter:
        ...
        response_query = response_query.filter(
            Supplier.supplier_name.ilike(f"%{supplier_name_filter}%")
        )
    if material_name_filter:
        response_query = response_query.filter(
            Material.material_name.ilike(f"%{material_name_filter}%")
        )
    if material_model_filter:
        response_query = response_query.filter(
            time_period_subquery.c.material_model.ilike(f"%{material_model_filter}%")
        )
    if material_specification_filter:
        response_query = response_query.filter(
            time_period_subquery.c.material_specification.ilike(
                f"%{material_specification_filter}%"
            )
        )
    if material_color_filter:
        response_query = response_query.filter(
            time_period_subquery.c.color.ilike(f"%{material_color_filter}%")
        )
    if order_rid_filter:
        response_query = response_query.filter(
            time_period_subquery.c.order_rid.ilike(f"%{order_rid_filter}%")
        )
    total_count = response_query.distinct().count()
    response_entities = (
        response_query.distinct()
        .limit(page_size)
    ...
    for (
        ...
        res = db_obj_to_res(
            material, Material, attr_name_list=INBOUND_SUMMARY_MATERIAL_COLUMNS
        )
...
@accounting_warehouse_bp.route(
    "/accounting/get_inventory_display_columns", methods=["GET"]
)
...
@accounting_warehouse_bp.route(
    "/accounting/get_inbound_display_columns", methods=["GET"]
)
...
@accounting_warehouse_bp.route(
    "/accounting/get_inbound_summary_display_columns", methods=["GET"]
)
...
@accounting_warehouse_bp.route(
    "/accounting/get_outbound_display_columns", methods=["GET"]
)
...
@accounting_warehouse_bp.route(
    "/accounting/createinboundexcelanddownload", methods=["GET"]
)
def create_excel_and_download():
    ...
    time_range_string = (
        date_range_filter_start + "至" + date_range_filter_end
        if date_range_filter_start and date_range_filter_end
        else "全部"
    )
    generate_accounting_inbound_excel(
        template_path,
        save_path,
        warehouse_filter,
...
@accounting_warehouse_bp.route(
    "/accounting/createinboundsummaryexcelanddownload", methods=["GET"]
)
def create_inbound_summary_excel_and_download():
    ...
    generate_accounting_summary_excel(
        template_path,
        save_path,
        warehouse_filter,
        supplier_name_filter,
...
@accounting_warehouse_bp.route(
    "/accounting/createinventoryexcelanddownload", methods=["GET"]
)
...
@accounting_warehouse_bp.route("/accounting/exportinventoryhistory", methods=["GET"])
def export_inventory_history():
    warehouse_filter = request.args.get("warehouseIdFilter", type=int)
    supplier_name_filter = request.args.get("supplierNameFilter")
    material_model_filter = request.args.get("materialModelFilter")
    material_name_filter = request.args.get("materialNameFilter")
    material_specification_filter = request.args.get("materialSpecificationFilter")
    material_color_filter = request.args.get("materialColorFilter")

    order_rid_filter = request.args.get("orderRidFilter")
    order_shoe_customer_name_filter = request.args.get("customerProductNameFilter")
    shoe_rid_filter = request.args.get("shoeRidFilter")
    not_zero_flag_filter = request.args.get("includeZeroFilter")
