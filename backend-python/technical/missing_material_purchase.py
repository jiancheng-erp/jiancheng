...
from constants import (
    MISSING_FLOW_ACTIVE_STATUSES,
    FIRST_PURCHASE_FINISH_STATUS,
    SECOND_PURCHASE_FINISH_STATUS,
    SIZE_BASED_TYPES,
    TYPE_TEXT,
    SHOESIZERANGE,
...
def fill_item_size_columns(poi: PurchaseOrderItem, arr: List[int]):
    arr = list(arr or [])
    if len(arr) < len(SHOESIZERANGE):
        arr += [0] * (len(SHOESIZERANGE) - len(arr))
...
def subq_order_shoe_flags(db, OrderShoeStatus):
    return (
        db.session.query(
            ...
            func.max(
                case(
                    (OrderShoeStatus.current_status == FIRST_PURCHASE_FINISH_STATUS, 1),
                    else_=0,
                )
            ).label("has8"),
            func.max(
                case(
                    (
                        OrderShoeStatus.current_status == SECOND_PURCHASE_FINISH_STATUS,
                        1,
                    ),
                    else_=0,
                )
...
# ================================= 1) 主表：已下发到总仓（可排除补采中） ================================
@missing_material_purchase_bp.route(
    "/missing_material_purchase/getwarehouseorders", methods=["GET"]
)
def get_warehouse_orders():
    ...
    sub_inprog_cnt = (
        ...
        .filter(WMR.status.in_(MISSING_FLOW_ACTIVE_STATUSES))
    ...
    if sort_prop == "issueDate":
        q = q.order_by(
            desc(Order.start_date) if sort_order == "desc" else Order.start_date
        )
    else:
    ...
    if order_ids:
        pend_rows = (
            ...
            .filter(
                WMR.order_id.in_(order_ids),
                WMR.status.in_(MISSING_FLOW_ACTIVE_STATUSES),
            )
            .group_by(WMR.order_id)
            .all()
        )
    ...
    def map_row(order, order_shoe, customer, shoe):
        return {
            ...
            "customerBrand": getattr(order, "customer_brand", None),
            "issueDate": order.start_date.isoformat() if order.start_date else None,
            "warehouseStatusText": "已下发",
            "warehouseStatusTag": "success",
            "pendingRequestCount": pending_map.get(order.order_id, 0),
            "orderId": order.order_id,
            "orderShoeId": order_shoe.order_shoe_id,
            "orderShoeTypeId": getattr(order_shoe, "order_shoe_type_id", None),
        }
...
# ========================== 2) 补采流程中订单 - 汇总数字 ==========================
@missing_material_purchase_bp.route(
    "/missing_material_purchase/inprogress/summary", methods=["GET"]
)
...
# =========================== 3) 补采流程中订单 - 列表 ===========================
@missing_material_purchase_bp.route(
    "/missing_material_purchase/inprogress", methods=["GET"]
)
def inprogress_list():
    page = request.args.get("page", 1, type=int)
    ...
    def map_row(r):
        (missing_record_id, order_id, status, created_at, order_rid, cust_name) = r
        s = str(status) if status is not None else ""
        return {
            "missingRecordId": missing_record_id,
            "orderId": order_id,
            "orderRid": order_rid,
...
# =========================== 4) 新建补采请求（含明细） ===========================
@missing_material_purchase_bp.route(
    "/missing_material_purchase/request", methods=["POST"]
)
...
def get_missing_purchase_record_detail():
    ...
    item_rows = (
        ...
        .join(
            OrderShoeType,
            OrderShoeType.order_shoe_type_id == WMRItem.order_shoe_type_id,
        )
        .join(ShoeType, ShoeType.shoe_type_id == OrderShoeType.shoe_type_id)
        .join(Color, Color.color_id == ShoeType.color_id)
        .join(Material, Material.material_id == WMRItem.material_id)
...
def _stage_text(status):
    mapping = {"0": "等待用量填写", "1": "等待采购用量", "2": "补采流程已完成"}
...
def usage_tasks():
    ...
    item_stat = (
        db.session.query(
            ...
            func.sum(case((WMRItem.approval_usage.isnot(None), 1), else_=0)).label(
                "filled_count"
            ),
        )
        ...
        data.append(
            {
                ...
                "pendingNodeText": (
                    "待填写用量" if (filled_cnt or 0) < (item_cnt or 0) else "已填写"
                ),
...
def purchase_amount_tasks():
    ...
    item_stat = (
        db.session.query(
            WMRItem.record_id.label("rid"),
            func.count(WMRItem.id).label("item_count"),
            func.sum(case((WMRItem.purchase_amount.isnot(None), 1), else_=0)).label(
                "filled_count"
            ),
        )
        .group_by(WMRItem.record_id)
    ...
    for row in items:
        ...
        if _is_size_based(model_item.material_type):
            ...
            if clean_arr is not None:
                ...
                if uu is not None:
                    model_item.unit_usage = (
                        Decimal(str(uu)) if not isinstance(uu, Decimal) else uu
                    )
                if au is not None:
                    model_item.approval_usage = (
                        Decimal(str(au)) if not isinstance(au, Decimal) else au
                    )
        else:
            # 普通材料：照常保存
            if uu is not None:
...
def get_or_create_purchase_order(
    order_id: int, order_shoe_id: Optional[int]
) -> PurchaseOrder:
    po = (
...
def fill_item_size_columns(po_item: PurchaseOrderItem, arr: List[int]):
    ...
    arr = list(arr or [])
    # pad/truncate 到和 SHOESIZERANGE 一致
    if len(arr) < len(SHOESIZERANGE):
        arr = arr + [0] * (len(SHOESIZERANGE) - len(arr))
    if len(arr) > len(SHOESIZERANGE):
        arr = arr[: len(SHOESIZERANGE)]
        ...
        if not ost:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": f"明细 {wmr_item.id} 缺少 order_shoe_type_id",
                    }
                ),
                400,
            )
    ...
    if missing:
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"BOM 不存在（bom_type=0）：order_shoe_type_id={','.join(missing)}",
                }
            ),
            400,
        )
    ...
    try:
        ...
        for wmr_item, pur_amt, size_arr in staged_rows:
            ...
            bi = BomItem(
                ...
                material_specification=getattr(
                    wmr_item, "material_specification", None
                ),
                ...
                pairs=None,
                production_instruction_item_id=getattr(
                    wmr_item, "production_instruction_item_id", None
                ),
            )
            db.session.add(bi)
            db.session.flush()
        ...
        # 按供应商分组创建分单 + PO Item
        buckets: Dict[
            Optional[int], List[Tuple[WMRItem, Decimal, Optional[List[int]]]]
        ] = {}
        for wmr_item, pur_amt, size_arr in staged_rows:
        ...
        for supplier_id, group_rows in buckets.items():
            ...
            for wmr_item, pur_amt, size_arr in group_rows:
                ...
                poi = PurchaseOrderItem(
                    ...
                    craft_name=getattr(wmr_item, "craft_name", None),
                    remark=getattr(wmr_item, "remark", None),
                    related_selected_material_storage=getattr(
                        wmr_item, "related_selected_material_storage", "[]"
                    ),
                )
                db.session.add(poi)
                db.session.flush()
