...
from sqlalchemy import func, or_, and_, desc, literal, Numeric, case, Integer, Select
...
from accounting.accounting_warehouse import name_mapping_inventory
import os
import time
from general_document.accounting_warehouse_history_excel import generate_accounting_warehouse_excel
...
def add_filter_to_query(query: Select, filters: dict, base_or_change: str = "base") -> Select:
    ...
    display_zero_inventory = filters.get("displayZeroInventory", "true").lower() == "true"
    ...
    if material_specification:
        query = query.where(SPUMaterial.material_specification.ilike(f"%{material_specification}%"))
    if material_color:
        query = query.where(SPUMaterial.color.ilike(f"%{material_color}%"))
    if customer_product_name:
        query = query.where(OrderShoe.customer_product_name.ilike(f"%{customer_product_name}%"))
    if order_rid:
...
def _paginate_msids(base_snapshot_date: date, target_date: date, filters: dict, paginate: bool):
    """
    ...
    warehouse_name = filters.get("warehouseNameFilter")
    supplier_name = filters.get("supplierNameFilter")
    material_name = filters.get("materialNameFilter")
    material_model = filters.get("materialModelFilter")
    material_specification = filters.get("materialSpecificationFilter")
...
def compute_inventory_as_of(target_date: date, filters: dict, paginate: bool) -> dict:
    ...
    if not base_snapshot_date:
        # 没有任何快照：用 target_date 当作“基准日”，只显示有变动的库存
        base_snapshot_date = target_date
    ...
    if start_change_date <= target_date:
        agg_table = (
            select(
                DailyMaterialStorageChange.material_storage_id.label("msid"),
                func.coalesce(func.sum(DailyMaterialStorageChange.inbound_amount), 0).label("inbound_sum"),
                func.coalesce(func.sum(DailyMaterialStorageChange.outbound_amount), 0).label("outbound_sum"),
                func.coalesce(func.sum(DailyMaterialStorageChange.net_change), 0).label("net_change_sum"),
            )
            .where(
                and_(
                    DailyMaterialStorageChange.snapshot_date >= start_change_date,
        ...
        if filters.get("displayZeroInventory", "true").lower() != "true":
            agg_table = agg_table.where(agg_table.c.net_change_sum > 0)

        agg_table = agg_table.subquery()
        ...
        change_stmt = (
            ...
            .join(MaterialStorage, MaterialStorage.material_storage_id == agg_table.c.msid)
            .join(SPUMaterial, SPUMaterial.spu_material_id == MaterialStorage.spu_material_id)
            .join(Material, SPUMaterial.material_id == Material.material_id)
            .join(MaterialType, MaterialType.material_type_id == Material.material_type_id)
            .join(MaterialWarehouse, MaterialWarehouse.material_warehouse_id == MaterialType.warehouse_id)
    ...
    latest_price_subq = (
        ...
        .where(DailyMaterialStorageChange.snapshot_date <= target_date)
        .group_by(DailyMaterialStorageChange.material_storage_id)
        .subquery()
    )
        ...
        items.append(
            {
                ...
                "outboundAmount": str(inbound_amount - current_amount),
                "currentAmount": str(current_amount),
                "inboundedItemTotalPrice": inbound_amount * price_info.get("average_price"),
                "currentItemTotalPrice": current_amount * price_info.get("average_price")
            }
        )
...
def export_inventory_history():
    ...
    warehouse_name = db.session.query(MaterialWarehouse.material_warehouse_name).filter(MaterialWarehouse.material_warehouse_id == warehouse_id).scalar()
