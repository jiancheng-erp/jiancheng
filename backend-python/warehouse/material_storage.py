...
def get_all_material_info():
    ...
    for row in response:
        ...
        obj = {
            ...
            + storage.pending_inbound
            - storage.pending_outbound,  # 允许出库数量 = 当前库存 + 待入库 - 待出库
        }
        for i in range(len(SHOESIZERANGE)):
            ...
            pending_inbound = (
                size_details[i].pending_inbound if i < len(size_details) else 0
            )
            pending_outbound = (
            ...
            obj[f"allowedOutboundAmount{i}"] = (
                current_amount + pending_inbound - pending_outbound
            )  # 允许出库数量 = 当前库存 + 待入库 - 待出库
        result.append(obj)
...
def get_size_material_storage_by_storage_id():
    ...
    for i, shoe_size in enumerate(SHOESIZERANGE):
        ...
        obj[f"allowedOutboundAmount{i}"] = (
            size_details[i].current_amount
            + size_details[i].pending_inbound
            - size_details[i].pending_outbound
        )
    return obj
...
def get_materials():
    ...
    for row in response:
        ...
        for i, shoe_size in enumerate(SHOESIZERANGE):
            ...
            obj[f"actualInboundAmount{i}"] = inbound_amount
            obj[f"currentAmount{i}"] = current_amount
            obj[f"remainingAmount{i}"] = (
                estimated_amount - inbound_amount - pending_inbound
            )
        result.append(obj)
    return {"result": result, "total": count_result}
...
def _create_outbound_record_details(items, outbound_record):
    ...
    for item in items:
        ...
        if outbound_record.outbound_type == 4:
            ...
            if (
                outbound_quantity
                > storage.current_amount
                + storage.pending_inbound
                - storage.pending_outbound
            ):
            ...
            for i, shoe_size in enumerate(SHOESIZERANGE):
                ...
                if (
                    ...
                    > size_detail[i].current_amount
                    + size_detail[i].pending_inbound
                    - size_detail[i].pending_outbound
                ):
                    error_message = json.dumps({"message": "出库数量大于库存数量"})
                    abort(Response(error_message, 400))
