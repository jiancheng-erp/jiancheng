...
def generate_accounting_warehouse_excel(template_path, save_path, warehouse_name, supplier_name, material_model,time_range, materials_data):
    ...
    for index, data in enumerate(materials_data, start=start_row):
        for j, col in enumerate(["materialWarehouse", "supplierName", "materialType", "materialName", 
                    "materialModel", "materialSpecification", "color", "orderRId", 
                    "customerProductName", "shoeRId", "inboundAmount", "outboundAmount",
                    "currentAmount", "unitPrice", "actualInboundUnit", "averagePrice", 
                    "itemTotalPrice"]):
