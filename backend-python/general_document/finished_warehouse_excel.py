...
from models import (
    Order, OrderShoe, OrderShoeType, Shoe, ShoeType, Color, Customer,
    FinishedShoeStorage,
    ShoeInboundRecord, ShoeInboundRecordDetail,
    ShoeOutboundRecord, ShoeOutboundRecordDetail
)
...
def _apply_common_filters(q, f):
    ...
    if f.get("customer_product_name"):
        q = q.filter(_safe_ilike(OrderShoe.customer_product_name, f["customer_product_name"]))
    if f.get("order_cid"):
...
def _filters_summary(filters: dict) -> str:
    ...
    if filters.get("start_date") or filters.get("end_date"):
        parts.append(f"时间: {filters.get('start_date') or '-'} ~ {filters.get('end_date') or '-'}")
...
def _apply_widths(ws, widths: list[int], padding: int = 2, min_w: int = 8, max_w: int = 48):
...
def _write_data_row(ws, row_idx: int, values: list, widths: list[int], center_cols: set[int] | None = None):
...
def build_finished_inbound_excel(filters: dict):
    ...
    if filters.get("inbound_rid"):
        q = q.filter(_safe_ilike(ShoeInboundRecord.shoe_inbound_rid, filters["inbound_rid"]))
    q = _apply_common_filters(q, filters)

    rows = q.all()

    wb = Workbook()  # 常规模式，支持合并单元格、列宽
    ws = wb.active
    ws.title = "成品入库记录"
...
def build_finished_outbound_excel(filters: dict):
    ...
    if filters.get("outbound_rid"):
        q = q.filter(_safe_ilike(ShoeOutboundRecord.shoe_outbound_rid, filters["outbound_rid"]))
    q = _apply_common_filters(q, filters)
...
def build_finished_inout_excel(filters: dict):
    ...
    if filters.get("inbound_rid"):
        in_q = in_q.filter(_safe_ilike(ShoeInboundRecord.shoe_inbound_rid, filters["inbound_rid"]))
    ...
    if filters.get("outbound_rid"):
        out_q = out_q.filter(_safe_ilike(ShoeOutboundRecord.shoe_outbound_rid, filters["outbound_rid"]))
    ...
    for order, shoe_rid_v, cust_prod_name, color_name, customer, record, detail in in_rows:
        _write_data_row(ws, r, [
            "入库",
            order.order_rid, order.order_cid, customer.customer_name, customer.customer_brand,
            cust_prod_name, shoe_rid_v, color_name,
            record.shoe_inbound_rid, _format_dt(record.inbound_datetime),
            detail.inbound_amount,
            detail.remark or "",
        ], widths, center_cols=center_cols)
        r += 1

    for order, shoe_rid_v, cust_prod_name, color_name, customer, record, detail in out_rows:
        qty = getattr(detail, "outbound_amount", None)
        if qty is None:
            qty = getattr(detail, "amount", None)
        _write_data_row(ws, r, [
            ...
            order.order_rid, order.order_cid, customer.customer_name, customer.customer_brand,
            cust_prod_name, shoe_rid_v, color_name,
            record.shoe_outbound_rid, _format_dt(record.outbound_datetime),
            qty,
            getattr(detail, "remark", "") or "",
        ], widths, center_cols=center_cols)
        r += 1
